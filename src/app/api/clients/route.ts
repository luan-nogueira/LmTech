...
import { Client, AdminConfig, LeadEstimate } from '@/types/client';
...
async function writeDatabase(data: Partial<DatabaseData>): Promise<DatabaseData> {
  const current = await readDatabase();
  const updated: DatabaseData = {
    clients: data.clients !== undefined ? data.clients : current.clients,
    adminConfig: data.adminConfig !== undefined ? { ...current.adminConfig, ...data.adminConfig } : current.adminConfig,
    leads: data.leads !== undefined ? data.leads : current.leads,
  };
